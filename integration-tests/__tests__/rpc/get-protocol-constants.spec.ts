...
import { ConstantsResponseProto023, ConstantsResponseProto025 } from '@taquito/rpc';
...
// every activation (Tallinn -> Ushuaia broke this file), so instead we assert the
// response decodes, that stable core fields are right, and that the current
// protocol's structural markers are present. Structural values verified identical
// on live shadownet + mainnet Proto25 (2026-07); feature-flag booleans are only
// type-checked because their values flip when a feature activates.
const expectCurrentProtocolConstants = (constants: ConstantsResponseProto025) => {
  ...
  // Use BigNumber.isBigNumber (not toBeInstanceOf): RPC builds BNs from its own
  // bignumber.js copy, so constructor identity fails across package boundaries
  // (same dual-copy issue vitest.setup.ts handles for toEqual).
  expect(constants.proof_of_work_nonce_size).toEqual(8);
  expect(constants.nonce_length).toEqual(32);
  expect(BigNumber.isBigNumber(constants.hard_gas_limit_per_operation)).toBe(true);
  expect(BigNumber.isBigNumber(constants.cost_per_byte)).toBe(true);
  // Proto25 (Ushuaia) structural markers.
  expect(constants.cache_layout_size).toEqual(5);
  expect(constants.dal_parametric.attestation_lags).toEqual([1, 2, 3, 4, 5]);
  // Proto25 marker fields: present with the right types.
...
CONFIGS().forEach(({ lib, rpc, networkType }) => {
  ...
  describe('Test fetching constants for all protocols on Mainnet', () => {
    ...
    // Mainnet tracks whichever protocol is live, so a frozen full-constants
    // fixture rots on every activation (Tallinn -> Ushuaia broke it here). Assert
    // the response decodes and exposes the current protocol's marker fields with
    // the expected shapes instead; the per-testnet blocks below keep exhaustive,
    // protocol-pinned fixtures for the networks that stay put.
  ...
  describe(`Fetch constants for testnet`, () => {
    // Tallinnnet/ushuaianet retired. Shadownet + mainnet-at-head exercise the
    // live Proto25/Ushuaia constant shape via expectCurrentProtocolConstants.
